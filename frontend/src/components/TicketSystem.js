          ...
          {selected.messages?.map((msg) => (
            <div key={msg.message_id} className={`flex ${msg.sender_type === 'admin' ? 'justify-start' : 'justify-end'}`}>
              <div className="max-w-[85%] px-3 py-2 rounded-xl text-sm"
                ...
                <p className="text-[10px] font-bold opacity-70 mb-0.5">{msg.sender_type === 'admin' ? '👮 Support' : '👤 You'}</p>
                <p>{msg.message}</p>
          ...
          {ticket.messages?.map((msg) => (
            <div key={msg.message_id} className={`flex ${msg.sender_type === 'admin' ? 'justify-start' : 'justify-end'}`}>
              <div className="max-w-[85%] px-3 py-2 rounded-xl text-xs"
                ...
                <p className="font-bold opacity-60 mb-0.5">{msg.sender_type === 'admin' ? '👮 Support' : '👤 You'}</p>
                <p>{msg.message}</p>
